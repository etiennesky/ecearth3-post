...
{
    echo
    echo "USAGE: ${0} -C <MY_SETUP> -R <run name> (options)"
    echo
...
while getopts C:R:y:t:feh option ; do
    case $option in
        C) MY_SETUP=${OPTARG};;
        R) RUN=${OPTARG};;
...
export POST_DIR=`echo ${POST_DIR} | sed -e "s|<RUN>|${RUN}|g"`
...
export DIAG_D=${DIR_TIME_SERIES}/${RUN}/ocean
...
ca=`\ls ${POST_DIR}/mon/Post_????/*_${cv_test}.nc | head -1` ; ca=`basename ${ca}` ;
export YEAR_INI=`echo ${ca} | sed -e "s/${RUN}_//g" -e "s/_${cv_test}.nc//g"`

ca=`\ls ${POST_DIR}/mon/Post_????/*_${cv_test}.nc | tail -1` ; ca=`basename ${ca}` ;
...
if [ ${IPREPHTML} -eq 0 ]; then
    ...
    while [ ${icontinue} -eq 1 ]; do
        ...
        ftst=${POST_DIR}/mon/Post_????/${RUN}_${jyear}_${cv_test}.nc
        ...
        if [ ${icontinue} -eq 1 ]; then
            ...
            for cvar in ${avars_2d[*]}; do
                ...
                f_in=${POST_DIR}/mon/Post_????/${RUN}_${jyear}_${cvar}_mean.nc
            ...
            f_in=${POST_DIR}/mon/Post_????/${RUN}_${jyear}_moc.nc
            ...
            cvar="iiceconc"
            f_in=${POST_DIR}/mon/Post_????/${RUN}_${jyear}_${cvar}.nc
