...
while getopts C:R:y:t:foeh option ; do
    case $option in
        C) MY_SETUP=${OPTARG};;
        R) RUN=${OPTARG};;
    ...
    while [ ${icontinue} -eq 1 ]; do
        ...
        if [ ${icontinue} -eq 1 ]; then
            ...
            for cvar in ${avars[*]}; do
                ...
                echo "cdo -R -t -fldmean ${POST_DIR}/mon/Post_????/${RUN}_${jyear}_${cvar}.nc ${cf_out}"
                cdo -R -fldmean ${POST_DIR}/mon/Post_????/${RUN}_${jyear}_${cvar}.nc tmp0.nc
                echo
